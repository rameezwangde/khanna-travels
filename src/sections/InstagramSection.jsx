...
export default function InstagramSection() {
  const { instagramFeed } = useContent();
  const images = [
      ...
      <div className="luxury-container">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {instagramFeed.map((post, index) => (
            <a key={post.caption} href={post.href || "https://www.instagram.com/khanna.travels/"} target="_blank" rel="noreferrer" className={`group relative overflow-hidden rounded-[1.6rem] border border-white/10 ${index === 0 ? "md:col-span-2 md:row-span-2" : ""} min-h-[260px]`}>
              <img src={post.image || images[index % images.length]} alt="" loading="lazy" className="absolute inset-0 h-full w-full object-cover transition duration-700 group-hover:scale-110" />
              <div className="absolute inset-0 bg-gradient-to-t from-navy via-navy/10 to-transparent" />
              <p className="absolute bottom-5 left-5 right-5 font-serif text-2xl font-bold text-ivory">{post.caption}</p>
            </a>
          ))}
        </div>
      </div>
